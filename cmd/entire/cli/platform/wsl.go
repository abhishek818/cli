...
// Detection is based on the contents of /proc/version which contains "microsoft" on WSL.
...
func WslpathWindows(linuxPath string) (string, error) {
	...
	//nolint:gosec // arguments are not shells, and linuxPath is a path, not a command
...
func WslpathLinux(winPath string) (string, error) {
	...
	//nolint:gosec // arguments are not shells, and winPath is a path, not a command
...
func windowsHomeDirViaCmdExe() (string, error) {
	...
	//nolint:gosec // Arguments are constants, not user input
